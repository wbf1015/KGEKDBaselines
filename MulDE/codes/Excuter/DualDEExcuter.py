...
class DualDEExcuter(object):
    def __init__(self, 
        ...
        self.args = args
        self.model = model(KGE=KGE, embedding_manager=embedding_manager, dualloss=dualloss, args=args)
    ...
    def save_model(self, model, args):
        ...
        torch.save({
            ...
            os.path.join(args.save_path, 'model_checkpoint')
        )
        
        # 单独保存entity和relation的embedding
        entity_embedding, relation_embedding = model.EmbeddingManager.get_embedding()
        state_dict = {
            'model_state_dict': {
                'origin_entity_embedding' : model.EmbeddingManager.origin_entity_embedding,
                'origin_relation_embedding' : model.EmbeddingManager.origin_relation_embedding,
                'entity_embedding': entity_embedding,
                'relation_embedding': relation_embedding
            }
        }
        torch.save(
            state_dict, os.path.join(args.save_path, 'embedding_checkpoint')
        )
